...
const App = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (found) {
      ...
      .catch((error) => {
        setNotify({message: `Information of ${found.name} has already removed from the server`, type: NotifyTypes.error})
        setTimeout(() => setNotify(null), 5000)
    ...
    personService
      ...
      .catch((error) => {
        setNotify({message: `failed to add ${newName} information to the server`, type: NotifyTypes.error})
        setTimeout(() => setNotify(null), 5000)
      })
  ...
  const removePerson = (event) => {
    ...
    personService
      ...
      .then(() => {
        ...
        setNotify({message: `Removed: ${id}`, type: NotifyTypes.remove})
        setTimeout(() => setNotify(null), 5000)
      })
      .catch((error) => {
        setNotify({message: `failed to remove ${id} information from the server`, type: NotifyTypes.error})
        setTimeout(() => setNotify(null), 5000)
      })
