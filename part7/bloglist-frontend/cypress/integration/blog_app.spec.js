...
describe('Blog App', function () {
  ...
  describe('Login', function () {
    it('succeeds with correct credentials', function () {
      ...
      cy.contains('Jone Doe logged-in successfully').should(
        'have.css',
        'color',
        'rgb(0, 128, 0)'
      )
    ...
    describe('When logged in', function () {
      ...
      it('A blog can be created', function () {
        cy.contains('new blog')
          ...
          .type('This is the blog title')
          .get('input[name="author"]')
          .type('the blog author')
          .get('input[name="url"]')
          .type('blogUrl.com')

        cy.get('button[type="submit"]').contains('create').click()

        cy.contains('This is the blog title the blog author').contains('view')
      })
      ...
      describe('When there are at least one blog', function () {
        ...
        it('can add like to a blog', function () {
          ...
          cy.get('@myBlog').get('button').contains('like').click()

          cy.get('@myBlog').contains('likes: 1')
        })
        ...
        it('blogs are displayed in order, depending on likes', function () {
          ...
          cy.get('@first').contains('like').click().wait(100).click()

          cy.get('.blog').then((blogs) => {
            cy.wrap(blogs[0]).contains('This is the third blog title')

            cy.wrap(blogs[1]).contains('This is the first blog title')

            cy.wrap(blogs[2]).contains('This is the second blog title')
          })
        })
      })
    })
